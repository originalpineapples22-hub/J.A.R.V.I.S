...
let settingsLoaded = false;   // has this form been filled from the server?
// Fields carrying a real secret: the server never sends the value back (see
// /api/settings), only the sentinel '(already set)' or ''. Fill the box from
// it directly — never with the sentinel itself — and $save() below skips any
// of these left blank, so an untouched box can never overwrite a stored key.
const SECRET = (id, v) => { $(id).value = (v && v !== '(already set)') ? v : ''; $(id).placeholder = v ? '•••• already set — leave blank to keep it' : $(id).placeholder; };
async function loadSettings() { try { const s = await api('/api/settings'); $('#s-name').value = s.operator_name; $('#s-ai-name').value = s.assistant_name || ''; $('#s-ai-style').value = s.assistant_style || ''; $('#s-provider').value = s.provider; $('#s-oai-url').value = s.openai_base_url; SECRET('#s-oai-key', s.openai_api_key); $('#s-oai-model').value = s.openai_model; $('#s-ollama').value = s.ollama_url; $('#s-ollama-model').value = s.ollama_model; $('#s-tz').value = s.timezone; $('#s-brief').value = s.briefing_hour;
    ...
    SECRET('#s-gemini', s.gemini_key);
    $('#s-use-ollama').checked = !!s.use_ollama; $('#s-prefer-local').checked = !!s.prefer_local; $('#s-budget').value = s.daily_call_budget || 900;
    SECRET('#s-groq', s.groq_api_key); $('#s-tavily').value = s.tavily_key || ''; $('#s-wolfram').value = s.wolfram_appid || '';
    ...
    $('#s-hooks').value = s.webhooks || '{}';
    settingsLoaded = true; } catch (_) { settingsLoaded = false; } }
...
$('#s-save').onclick = async () => {
  ...
  const fields = {operator_name: $('#s-name').value, assistant_name: $('#s-ai-name').value, assistant_style: $('#s-ai-style').value, provider: $('#s-provider').value, openai_base_url: $('#s-oai-url').value, openai_api_key: $('#s-oai-key').value, openai_model: $('#s-oai-model').value, ollama_url: $('#s-ollama').value, ollama_model: $('#s-ollama-model').value, timezone: $('#s-tz').value, briefing_hour: parseInt($('#s-brief').value || '8'),
      gemini_key: $('#s-gemini').value, groq_api_key: $('#s-groq').value,
      use_ollama: $('#s-use-ollama').checked, prefer_local: $('#s-prefer-local').checked, daily_call_budget: parseInt($('#s-budget').value || '900'), tavily_key: $('#s-tavily').value, wolfram_appid: $('#s-wolfram').value,
      ...
      webhooks: $('#s-hooks').value};
  // The panel opens empty when there is no token yet, so its blank boxes are
  // "unknown", not "clear this". Posting them wiped every stored API key.
  // Until the form has actually been filled from the server, send only what
  // was typed; once loaded, blanks are a real instruction to clear.
  const payload = settingsLoaded ? fields
    : Object.fromEntries(Object.entries(fields).filter(([, v]) => v !== '' && v !== null && v !== undefined));
  // The three real-secret fields are the exception even after loading: the
  // server never sends their value back (only '(already set)' or ''), so the
  // box is always shown blank when a key already exists — leaving it blank
  // means "unchanged", not "delete it", or every unrelated save (renaming the
  // operator, changing the budget) would silently erase whichever key was
  // already stored.
  for (const id of ['gemini_key', 'groq_api_key', 'openai_api_key']) {
    if (payload[id] === '') delete payload[id];
  }
  try { await api('/api/settings', {method: 'POST', body: JSON.stringify(payload)});
    await loadSettings();          // the form now mirrors the server, so later saves are complete
    $('#s-msg').textContent = 'Saved. All systems online, sir.'; if (!wsReady) connect(); refresh(); setTimeout(() => modal.classList.add('hidden'), 800); } catch (e) { $('#s-msg').textContent = (e && e.message === 'unauthorized')
